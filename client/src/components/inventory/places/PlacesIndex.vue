    ...
    <div class="p-8 overflow-y-auto">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">Orte</h1>
                <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">Eine Auflistung aller angelegten Orte, an denen Inventar gelagert wird.</p>
            </div>
            <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
                <router-link to="/inventory/places/add" type="button" class="btn-primary">
                    <svg-icon type="mdi" :path="mdiMapMarkerPlus" class="-ml-0.5 h-5 w-5" aria-hidden="true"></svg-icon>
                    Ort hinzufügen
                </router-link>
            </div>
        </div>
                            ...
                            <tbody>
                                <tr v-for="place in places" :key="place.id">
                                    ...
                                    <td class="whitespace-nowrap text-right">
                                        <router-link :to="'/inventory/places/edit/' + place.id" type="button" class="btn-table-primary">
                                            <svg-icon type="mdi" :path="mdiFileEdit" class="-ml-0.5 h-5 w-5" aria-hidden="true"></svg-icon>
                                            Bearbeiten
                                        </router-link>
                                        <button @click="openDeleteDialog(place.title)" type="button" class="btn-table-danger ml-2">
                            ...
                            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <div class="sm:flex sm:items-start">
                                    ...
                                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                        ...
                                        <div class="mt-2">
                                            <p class="text-sm text-gray-500">Bist du dir sicher, dass du den Ort <b>{{ currentPlaceName }}</b> löschen möchtest? Dieser wird noch in <b>{{ currentPlaceUsageCount }}</b> Inventar-Gegenständen verwendet.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
const currentPlaceUsageCount = ref()
const open = ref(false)

const getPlaces = () => {
    const url = '/api/inventory/places/index'
    axios
        .get(url)
        .then((response: any) => {
